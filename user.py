...
Enter the option: 
    ...
    7. Update a record using id or user.
...
COLUMN_INPUT_STRING =f""" which column would you like to update?. please make sure column is one of the following: {COLUMNS}"""
...
def insert_users(conn, db_data:list[tuple])->None:
...
def update_user(conn, column_name:str, user_id:int,column_value:str | int)->None:
    """update single records to table
...
def delete_user_id(conn, user_id:int)->None:
    ...
    conn.commit()
    print("Successfully deleted records from user table.")
...
def main():
    ...
    while True:
        ...
        if user_input == "1":
            ...
            if confirmation_lower() in ["y","yes"]:
                delete_all_records()
